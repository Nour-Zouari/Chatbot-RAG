...
model="text-embedding-3-large"  #modèle pour générer les embeddings
...
def calculate_embeddings(corpus: str) -> list[float]:
    ...
    data = {"input": corpus, "model": "grok-embedding-1"}  # modèle hypothétique
    response = requests.post(GROK_API_URL, json=data, headers=headers)
    response_json = response.json()
    return response_json["embedding"]  # clé à vérifier selon Grok
